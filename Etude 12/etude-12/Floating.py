...
def ie32converter(sign, exponent, fraction):
    if((exponent == 255) & (fraction !=0)):
        ...
        temp3 = sign * (-1*fraction) * 16 + exponent
        temp2 = (-1)**sign * 2 **(exponent-64) * fraction
...
"""
def floatreader(file_in, file_out, f):
    length = 0
    try:
        """
                ...
                while True:
                    ...
                    if(x):
                        ...
                        if f == 's':
                            ...
                            f_point = ibm32converter(int(Y, 16))
...
"""
def doublereader(file_in, file_out, f):
    length = 0
    try:
        """
        The b when opening a file indicates binary
                ...
                while True:
                    ...
                    if(x):
                        ...
                        while i > 0:
                            ...
                            i -= 2
                        if f == 's':
                            f_point = ibm64converter(int(Y, 16))
                            #This line formats the string correctly
                            result = struct.pack('f', f_point)
                        else:
...
def singleToDouble(file_in, file_out, f):
    length = 0
    try:
        """
        The b when opening a file indicates binary
        ...
        with open(file_in, "rb") as streamIn:
            with open(file_out, "wb") as streamOut:
                while True:
                    #Reads in 4 bytes of data from the input file
                    x = streamIn.read(4)
                    #Adds 4 bytes of data to the input to create 8 bytes which is required for double precision
                    if(x):
                        length += 1
                        x += bytes('\x00\x00\x00\x00', 'utf-8')
                        Y = ""
                        """
                        This section  of code converts the input into hexadecimal format
                        """
                        for y in x:
                            if y == 0:
                                Y += "00"
                                ...
                                Y += hex(y)[2:]
                        """
                        This section of code is used to reverse the order of the input
                        This ensures that the value is in the correct order when it is assigned to the different
                        sections of a IEEE value.
                        """
                        reverseY = ""
                        i = len(Y) -1
                        while i > 0:
...
def inputtype(userinput):
    ...
    if(len(userinput) < 4 or len(userinput) > 4):
        ...
        if(f == 's'):
            if(o == 's'):
                return(floatreader(file_in, file_out, f))
            else:
                return(singleToDouble(file_in, file_out, f))
        elif(f == 'd'):
            return(doublereader(file_in, file_out, f))
        else:
